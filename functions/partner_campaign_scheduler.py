...
def _execute_partner_campaign_scheduler(request_id: str, start_time: datetime, trigger_type: str = "timer") -> dict:
    ...
    try:
        ...
        for campaign_num, campaign in enumerate(qualified_campaigns, 1):
            ...
            logging.info(f"📦 [PARTNER-SCHEDULER] Step 3.{campaign_num}.2: Creating batches (max 20 per batch)...")
            logging.info(f"📦 [PARTNER-SCHEDULER] Step 3.{campaign_num}.2: Calling member_service.create_batches()...")
            batches = member_service.create_batches(eligible_members, batch_size=20)
