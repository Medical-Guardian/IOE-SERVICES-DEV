...
from ..utils.config import (KEY_VAULT_URL, BLAND_AI_BATCH_URL, GET_CAMPAIGN_CONFIG_QUERY,
...
class BlandAIService:
    ...
    def create_outreach_batch(self, campaign_id: str, member_count: int) -> str:
        ...
        if not campaign_id or not isinstance(member_count, int) or member_count < 0:
            logger.error(f"❌ [BlandAIService] Invalid input: campaign_id={campaign_id}, member_count={member_count}")
    ...
    def build_bland_payload(self, config: Dict, members_with_attempts: List[Dict], batch_id: str) -> Dict:
        ...
        logger.info(f"🏗️ [BlandAIService] Building Updated Bland AI payload for {len(members_with_attempts)} members")
        try:
            ...
            logger.info(f"✅ [BlandAIService] Updated payload built: {len(call_objects)} call objects")
            return payload
        except Exception as e:
    ...
    def call_bland_ai_api(self, payload: Dict, api_key: str) -> Dict:
        ...
        logger.info(f"🚀 [BlandAIService] Making Bland AI API call with {len(payload.get('call_objects', []))} calls")
        headers = {
    ...
    def update_batch_with_vendor_id(self, batch_id: str, vendor_batch_id: str):
        ...
        logger.info(f"🔄 [BlandAIService] Updating batch {batch_id} with vendor ID: {vendor_batch_id}")
        try:
            self.db_service.execute_query(UPDATE_BATCH_VENDOR_ID_QUERY, (vendor_batch_id, batch_id), fetch_results=False)
            logger.info(f"✅ [BlandAIService] Batch updated successfully")
        except Exception as e:
    ...
    def update_batch_failed(self, batch_id: str, error_message: str):
        ...
        try:
            ...
            logger.info(f"✅ [BlandAIService] Batch failure updated")
        except Exception as e:
