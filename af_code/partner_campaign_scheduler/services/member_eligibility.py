...
class MemberEligibilityService:
    ...
    def get_eligible_members(self, campaign: QualifiedCampaign) -> List[EligibleMember]:
        ...
        for member_data in members_data:
            ...
            logger.info(
                f"   ⏰ Current time: {member_data.get('member_current_time')} (SQL-calculated)"
            )
            logger.info(
                f"   📅 Current day: {member_data.get('member_current_day')} (SQL-calculated)"
            )
    ...
    def create_batches(
        ...
        logger.info(
            f"📦 [MEMBER-ELIGIBILITY] Created {len(batches)} batches from {len(members)} members"
        )
        return batches
...
CASE m.timezone
    ...
    def _build_query_parameters(self, campaign: QualifiedCampaign) -> tuple:
        ...
        logger.debug(
            f"🔄 [MEMBER-ELIGIBILITY] Timezone conversion: '{operating_tz}' → '{operating_tz_windows}' (for SQL)"
        )
