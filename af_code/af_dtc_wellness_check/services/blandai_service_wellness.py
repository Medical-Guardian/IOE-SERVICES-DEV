...
class BlandAIServiceWellness:
    ...
    def create_outreach_batch(self, campaign_id: str, member_count: int) -> str:
        ...
        if not campaign_id or not isinstance(member_count, int) or member_count < 0:
            logger.error(
                f"❌ [BlandAIServiceWellness] Invalid input: campaign_id={campaign_id}, member_count={member_count}"
            )
    ...
    def create_outreach_attempts(self, qualified_members: List[Dict], batch_id: str) -> List[str]:
        ...
        logger.info(
            f"📝 [BlandAIServiceWellness] Creating {len(qualified_members)} outreach attempts"
        )
        attempt_ids = []
        try:
    ...
    def get_members_with_attempts(self, batch_id: str) -> List[Dict]:
        ...
        logger.info(
            f"👥 [BlandAIServiceWellness] Getting members with attempts for batch: {batch_id}"
        )
        try:
            members = self.db_service.execute_query(GET_MEMBERS_WITH_ATTEMPTS_QUERY, (batch_id,))
            logger.info(
                f"✅ [BlandAIServiceWellness] Retrieved {len(members)} members with attempts"
            )
            return members
        except Exception as e:
            logger.error(
                f"💥 [BlandAIServiceWellness] Error getting members with attempts: {str(e)}"
            )
            raise
    ...
    def update_batch_with_vendor_id(self, batch_id: str, vendor_batch_id: str):
        ...
        logger.info(
            f"🔄 [BlandAIServiceWellness] Updating batch {batch_id} with vendor ID: {vendor_batch_id}"
        )
        try:
            self.db_service.execute_query(
                UPDATE_BATCH_VENDOR_ID_QUERY,
            ...
            logger.error(
                f"💥 [BlandAIServiceWellness] Error updating batch with vendor ID: {str(e)}"
            )
            raise
    ...
    def update_batch_failed(self, batch_id: str, error_message: str):
        ...
        logger.info(
            f"❌ [BlandAIServiceWellness] Marking batch {batch_id} as failed: {error_message}"
        )
        try:
            self.db_service.execute_query(
                UPDATE_BATCH_FAILED_QUERY,
