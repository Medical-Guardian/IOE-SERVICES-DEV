...
class PartnerCampaignConfig:
    ...
    # Filename validation
    expected_filename_pattern: str = r'^([A-Za-z0-9]+)_([A-Za-z0-9\/\s]+)_(\d{8})(_[A-Za-z0-9]+)?\.csv$'
...
class DataCleanerAndValidator:
    ...
    @staticmethod
    def _to_e164_phone(phone_str: str, row_number: int, field_name: str) -> Tuple[Optional[str], Optional[ValidationError]]:
        ...
        if len(digits) == 10:
            ...
            if phone_str != e164_phone:
                warning = ValidationError(category="Format", error_type="PhoneNumberReformatted",
                                          message=f"Row {row_number}: {field_name} was reformatted to E.164.",
                                          severity=ValidationSeverity.WARNING, field=field_name, row_number=row_number)
        ...
        error = ValidationError(category="Format", error_type="InvalidPhoneNumber",
                                message=f"Row {row_number}: {field_name} is not a valid phone number.",
                                severity=ValidationSeverity.ERROR, field=field_name, error_value=phone_str,
                                row_number=row_number)
        return None, error

    @staticmethod
    def _to_iso8601_date(date_str: str, row_number: int, field_name: str) -> Tuple[
        Optional[str], Optional[ValidationError]]:
        ...
        for fmt in ('%m/%d/%Y', '%m-%d-%Y', '%Y/%m/%d'):
            try:
                ...
                warning = ValidationError(category="Format", error_type="DateReformatted",
        ...
        error = ValidationError(category="Format", error_type="InvalidDate",
                                message=f"Row {row_number}: {field_name} is not a valid or recognized date.",
                                severity=ValidationSeverity.ERROR, field=field_name, error_value=date_str,
                                row_number=row_number)
        return None, error

    def clean_and_validate_dataframe(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, List[ValidationError]]:
        """
            ...
            for col in PartnerCampaignRules.NON_NULLABLE_COLUMNS:
                ...
                if pd.isna(row.get(col)) or str(row.get(col, '')).strip() == '':
                    all_errors.append(ValidationError(
                        category="Data", error_type="MissingRequiredValue",
                        message=f"Row {row_number}: Mandatory field '{col}' cannot be empty.",
                        severity=ValidationSeverity.ERROR, field=col, row_number=row_number
                    ))

        # --- DATA CLEANING & TRANSFORMATION ---
        name_columns = ['member_first_name', 'member_last_name', 'caregiver_first_name', 'caregiver_last_name']
        ...
        for col in phone_columns:
            results = [self._to_e164_phone(val, idx + 2, col) for idx, val in cleaned_df[col].astype(str).items()]
            cleaned_df[col] = [res[0] for res in results]
            all_errors.extend([res[1] for res in results if res[1] is not None])

        results = [self._to_iso8601_date(val, idx + 2, 'member_dob') for idx, val in
                   cleaned_df['member_dob'].astype(str).items()]
        ...
        for idx, row in cleaned_df.iterrows():
            ...
            if not str(row.get('salesforce_account_number', '')).isdigit():
                all_errors.append(ValidationError(category="Format", error_type="InvalidSalesforceNumber",
                                                  message=f"Row {row_number}: salesforce_account_number must contain only digits.",
                                                  severity=ValidationSeverity.ERROR, field='salesforce_account_number',
                                                  error_value=str(row.get('salesforce_account_number')),
                                                  row_number=row_number))

            if not re.match(r'^America\/[A-Za-z_]+$', str(row.get('member_timezone', ''))):
                all_errors.append(ValidationError(category="Format", error_type="InvalidTimezone",
                                                  message=f"Row {row_number}: member_timezone is not a valid Olson timezone string.",
                                                  severity=ValidationSeverity.ERROR, field='member_timezone',
                                                  error_value=str(row.get('member_timezone')), row_number=row_number))

            lang = str(row.get('language_pref', '')).upper()
            if lang and lang not in ['EN', 'ES']:
                all_errors.append(ValidationError(category="Format", error_type="InvalidLanguage",
                                                  message=f"Row {row_number}: language_pref must be EN or ES.",
                                                  severity=ValidationSeverity.ERROR, field='language_pref',
                                                  error_value=str(row.get('language_pref')), row_number=row_number))

            email_fields = ['member_email', 'caregiver_email', 'healthcare_email']
            for field in email_fields:
                email = str(row.get(field, ''))
                if email and not re.match(r'^[^\s@]+@[^\s@]+\.[^\s@]+$', email):
                    all_errors.append(ValidationError(category="Format", error_type="InvalidEmail",
                                                      message=f"Row {row_number}: {field} is not a valid email address.",
                                                      severity=ValidationSeverity.ERROR, field=field, error_value=email,
                                                      row_number=row_number))
...
class PartnerCampaignRules:
    ...
    EXPECTED_COLUMNS = [
        'partner_name', 'campaign_name_source', 'language_pref', 'salesforce_account_number',
        'healthcare_member_id', 'member_first_name', 'member_last_name', 'member_phone_number',
        'member_timezone', 'member_dob', 'member_email', 'member_address_street',
        'member_address_city', 'member_address_state', 'member_address_zip', 'member_address_country',
        'caregiver_first_name', 'caregiver_last_name', 'caregiver_phone_number', 'caregiver_email',
        'channel_type', 'device_udi', 'device_name', 'is_device_callable',
        'device_phone_number', 'checkin_time', 'medscope_contact_id', 'mco_id',
        'campaign_parameters', 'healthcare_email'
    ]

    # The 15 columns that MUST exist in every file. File is rejected if any are missing.
    MANDATORY_COLUMNS = [
        'partner_name', 'campaign_name_source', 'salesforce_account_number', 'member_first_name',
        'member_last_name', 'member_phone_number', 'member_timezone', 'member_dob',
        'member_email', 'caregiver_first_name', 'caregiver_last_name', 'caregiver_phone_number',
        'caregiver_email', 'campaign_parameters', 'healthcare_email'
    ]
...
def download_blob_as_dataframe(blob_name: str, container_name: str) -> pd.DataFrame:
    ...
    logging.info(f"Successfully downloaded blob '{blob_name}' ({file_size_mb:.2f} MB). Now parsing into DataFrame.")
...
def move_blob(blob_name: str, source_folder: str, target_folder: str, container_name: str):
    ...
    logging.info(f"Attempting to move blob from '{source_blob_path}' to '{target_blob_path}' in container '{container_name}'.")
...
class FileNameValidator:
    ...
    @staticmethod
    def validate(file_name: str) -> Tuple[bool, Optional[ValidationError], Optional[Dict[str, str]]]:
        """
...
class ColumnValidator:
    ...
    def validate(headers: List[str]) -> Tuple[List[ValidationError], List[str]]:
        ...
        if missing_mandatory:
            for column in missing_mandatory:
                errors.append(ValidationError(
                    category="Schema", error_type="MissingMandatoryColumn",
                    message=f"Mandatory column '{column}' is missing from the file.",
                    severity=ValidationSeverity.ERROR, field=column
                ))
        ...
        for column in missing_optional:
            errors.append(ValidationError(
                category="Schema", error_type="MissingOptionalColumn",
                message=f"Optional column '{column}' was not found. It will be treated as null.",
                severity=ValidationSeverity.WARNING, field=column
            ))
        ...
        for column in unexpected_columns:
            errors.append(ValidationError(
                category="Schema", error_type="UnexpectedColumn",
...
class FormatValidator:
    ...
    def validate_row(row_data: Dict[str, str], row_number: int) -> List[ValidationError]:
        ...
        for field, rules in PartnerCampaignRules.FORMAT_PATTERNS.items():
        ...
        for field, rules in PartnerCampaignRules.ALLOWED_VALUES.items():
            value = str(row_data.get(field, '')).strip()
...
class CareGapsValidator:
    ...
    def _get_active_care_gaps(self) -> Dict[str, bool]:
        ...
        try:
            ...
            with db_logger.get_connection() as conn:
                ...
                # LOGGING: Log success
                logging.info(f"Successfully fetched and cached {len(self._care_gaps_cache)} care gaps.")
                return self._care_gaps_cache
    ...
    def validate_row(self, row_data: Dict[str, str], row_number: int) -> Tuple[
        List[ValidationError], List[str], List[str]]:
        """
        ...
        try:
            ...
            errors.append(ValidationError(
                ...
                severity=ValidationSeverity.ERROR,
                field="campaign_parameters",
                error_value=campaign_params_str[:100] + "..." if len(
                    campaign_params_str) > 100 else campaign_params_str,
...
class DatabaseLogger:
    ...
    def log_care_gap_stats(self, file_processing_id: int, care_gap_summary: Dict[str, Dict[str, int]]):
                ...
                with self.get_connection() as conn:
                    ...
                    for care_gap_name, stats in care_gap_summary.items():
                        ...
                        cursor.execute("""
                            ...
                            VALUES (%s, %s, %s, %s)
                        """, (file_processing_id, care_gap_name, stats['active_count'], stats['total_count']))
                    conn.commit()
                    logging.info(f"Successfully logged {len(care_gap_summary)} care gap summary records.")
            except Exception as e:
    ...
    def get_connection(self):
        ...
        try:
            ...
            logging.info(f"Connecting to database '{conn_kwargs.get('database')}' on server '{conn_kwargs.get('server')}'...")
    ...
    def log_row_validation_results(self, file_processing_id: int, row_results: List[Dict]):
        ...
        try:
            ...
            logging.info(f"Logging {len(row_results)} row validation results to the database for file_processing_id {file_processing_id}...")
            with self.get_connection() as conn:
                cursor = conn.cursor()
                for result in row_results:
                    ...
                    """, (file_processing_id, result['row_number'], result['status'],
                          result['errors_count'], result['warnings_count'], result['partner_name'],
                          result['campaign_name_source'], result['salesforce_account_number'],
                          json.dumps(result['active_care_gaps']), result.get('healthcare_member_id', ''))) # Added get() for safety
                conn.commit()
    ...
    def log_row_level_errors(self, file_processing_id: int, errors: List[ValidationError]):
        ...
        try:
            # LOGGING: Log before inserting
            logging.info(f"Logging {len(errors)} row-level errors to the database for file_processing_id {file_processing_id}...")
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                for error in errors:
                    ...
                    """, (file_processing_id, error.row_number, error.category, error.error_type, error.field,
                          error.message, error.error_value, error.expected_value, error.severity))
    ...
    def log_file_processing_start(self, file_name: str, original_path: str, final_path: str,
                                  file_size: int, partner_name: str, campaign_name: str) -> int:
        """
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                """, (file_name, original_path, final_path, file_size,
                               partner_name, campaign_name, ProcessingStatus.PROCESSING))

                cursor.execute("SELECT @@IDENTITY")
                file_processing_id = cursor.fetchone()[0]
                conn.commit()

                logging.info(f"Successfully created file processing log entry with ID: {file_processing_id}")
    ...
    def log_file_level_errors(self, file_processing_id: int, errors: List[ValidationError]):
        ...
        try:
            # LOGGING: Log before inserting
            logging.info(f"Logging {len(errors)} file-level errors to the database for file_processing_id {file_processing_id}...")
            with self.get_connection() as conn:
                cursor = conn.cursor()
                ...
                for error in errors:
                    ...
                    """, (file_processing_id, error.category, error.error_type, error.field,
                                   error.message, error.error_value, error.expected_value, error.severity))
...
class PartnerCampaignProcessor:
    ...
    def process_file(self, file_name: str, container_name: str) -> ProcessingResult:
        ...
        try:
            ...
            if not name_valid:
                return ProcessingResult(success=False, message=f"Filename validation failed: {name_error.message}",
                                        validation_errors=[name_error])

            partner_name, campaign_name = name_components['partner_name'], name_components['campaign_name']
            ...
            file_processing_id = self.db_logger.log_file_processing_start(file_name, original_path, validated_path,
            ...
            if missing_mandatory:
                ...
                self._update_file_processing_completion(file_processing_id, ProcessingStatus.REJECTED, len(df), 0,
                                                        len(df), 0)
            ...
            error_rows_set = set(
                e.row_number for e in row_level_validation_errors if e.severity == ValidationSeverity.ERROR)
            invalid_rows = len(error_rows_set)
            valid_rows = total_rows - invalid_rows
            final_status = ProcessingStatus.COMPLETED
            final_message = f"Processing completed with {invalid_rows} invalid rows out of {total_rows} total."
            ...
            warning_rows_count = len(
                set(e.row_number for e in row_level_validation_errors if e.severity == ValidationSeverity.WARNING))
            self._update_file_processing_completion(file_processing_id, final_status, total_rows, valid_rows,
                                                    invalid_rows, warning_rows_count)
            ...
            return ProcessingResult(success=True, message=final_message, records_processed=total_rows,
                                    records_succeeded=valid_rows, records_failed=invalid_rows,
                                    duration_seconds=duration)

        except Exception as e:
            logging.exception("A critical, unhandled exception occurred.")
            if 'file_processing_id' in locals() and file_processing_id:
                self._update_file_processing_completion(file_processing_id, ProcessingStatus.FAILED, 0, 0, 0, 0)
            return ProcessingResult(success=False, message=f"Critical error in processing: {e}", error_details=str(e))
    ...
    def _create_row_results(self, df: pd.DataFrame, all_errors: List[ValidationError]) -> List[Dict[str, Any]]:
        """
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                if pd.notna(params_str) and params_str:
                    params = json.loads(params_str)
                    active_care_gaps = [k for k, v in params.items() if
                                        k.endswith('_import_flag') and str(v).upper() == 'Y']
            except (json.JSONDecodeError, TypeError):
    ...
    def _validate_single_row(self, row_data: Dict[str, str], row_number: int) -> List[ValidationError]:
        """
        ...
        try:
            ...
            all_errors.append(ValidationError(
                category="CareGap", error_type="ValidatorCrash",
                message=f"Row {row_number}: Care gaps validation crashed: {str(e)}",
                severity=ValidationSeverity.WARNING, field="campaign_parameters",
                row_number=row_number
            ))

        return all_errors

    def _update_file_processing_completion(self, file_processing_id: int, status: str,
                                           total_rows: int, valid_rows: int, invalid_rows: int, warning_rows: int):
        """Educational: Updates the file processing log with final results"""
        try:
            # LOGGING: Log before update
            logging.info(f"Updating final status for file_processing_id {file_processing_id} to '{status}'.")
            with self.db_logger.get_connection() as conn:
                ...
                """, (status, total_rows, valid_rows, invalid_rows, warning_rows, file_processing_id))

                conn.commit()
                logging.info(f"Successfully updated final status for file_processing_id: {file_processing_id}.")
...
def process_partner_campaign_file_complete(
    ...
    try:
        ...
        summary = {
            "file_name": file_path, "success": result.success, "message": result.message,
            "records_processed": result.records_processed, "records_succeeded": result.records_succeeded,
            "records_failed": result.records_failed, "duration_seconds": round(result.duration_seconds, 2),
            "error_rate_percent": (result.records_failed / result.records_processed * 100) if result.records_processed > 0 else 0,
        ...
        return False, error_msg, {
            "file_name": file_path, "success": False,
