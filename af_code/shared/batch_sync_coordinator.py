...
class BatchSyncCoordinator:
    ...
    async def execute_with_lock(self, operation_name: str, max_duration_minutes: int = 25) -> None:
        ...
        try:
            ...
            if not lock_acquired:
                ...
                logger.info(f"🔒 [BATCH-COORDINATOR] This prevents overlapping reconciliation processes")
                return
    ...
    async def _execute_batch_reconciliation(self) -> None:
        ...
        logger.info(f"📦 [BATCH-COORDINATOR] Found {len(stale_batches)} batches requiring reconciliation")
        ...
        for batch in stale_batches:
            try:
                ...
                logger.error(f"❌ [BATCH-COORDINATOR] Error reconciling batch {batch.get('vendor_batch_id', 'unknown')}: {str(e)}")
                batches_failed += 1
    ...
    async def _get_stale_batches(self, stale_threshold_hours: int = 2) -> List[Dict[str, Any]]:
        ...
        """
        logger.info(f"🔍 [BATCH-COORDINATOR] Looking for batches stale for more than {stale_threshold_hours} hours...")
              ...
              AND (
                  ...
                  NOT EXISTS (
                      ...
                      WHERE oa.batch_id = ob.batch_id
                        AND oa.updated_ts >= DATEADD(hour, -%s, SYSDATETIMEOFFSET())
                  )
                  OR
                  -- Batch is old enough to be considered stale
                  ob.submitted_ts <= DATEADD(hour, -%s, SYSDATETIMEOFFSET())
                  OR
    ...
    async def _reconcile_single_batch(self, batch: Dict[str, Any]) -> bool:
        ...
        logger.info(f"🔍 [BATCH-COORDINATOR] Checking batch: {vendor_batch_id} (Campaign: {campaign_name})")
    ...
    async def _acquire_sync_lock(self, operation_name: str, max_duration_minutes: int, instance_id: str) -> bool:
        ...
        try:
            ...
            if acquired:
                logger.info(f"🔓 [BATCH-COORDINATOR] Successfully acquired lock '{operation_name}' for {max_duration_minutes} minutes")
            else:
                logger.info(f"🔒 [BATCH-COORDINATOR] Lock '{operation_name}' already held by another process")
    ...
    async def _release_sync_lock(self, operation_name: str) -> None:
        ...
        try:
            rows_affected = self.db_service.execute_query(query, (operation_name,), fetch_results=False)
            
            if rows_affected > 0:
                logger.info(f"🔓 [BATCH-COORDINATOR] Successfully released lock '{operation_name}'")
            else:
                logger.warning(f"⚠️ [BATCH-COORDINATOR] Lock '{operation_name}' was not found (may have expired)")
