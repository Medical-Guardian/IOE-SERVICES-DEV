...
class BlandAIBatchMonitor:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        if not self.api_key:
            logger.warning(
                "⚠️ [BATCH-MONITOR] BlandAIkey not configured in Key Vault - Batch monitoring disabled"
            )
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status != 200:
                        ...
                        logger.error(
                            f"❌ [BATCH-MONITOR] API error {response.status}: {response_text}"
                        )
                        return None
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status != 200:
                        response_text = await response.text()
                        logger.error(
                            f"❌ [BATCH-MONITOR] API error getting logs {response.status}: {response_text}"
                        )
                        return None
                    ...
                    logger.info(
                        f"✅ [BATCH-MONITOR] Retrieved {len(logs)} log entries for batch: {batch_id}"
                    )
                    return logs
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    if response.status != 200:
                        response_text = await response.text()
                        logger.error(
                            f"❌ [BATCH-MONITOR] API error getting batch list {response.status}: {response_text}"
                        )
                        return None
                    ...
                    logger.info(
                        f"✅ [BATCH-MONITOR] Found {len(recent_batches)} recent batches out of {len(all_batches)} total"
                    )
                    return recent_batches

        except aiohttp.ClientTimeout:
            logger.error("⏰ [BATCH-MONITOR] Timeout getting recent batches")
            return None
    ...
    async def _analyze_batch_status(
    ...
    def check_api_health(self) -> bool:
        ...
        try:
            ...
            is_healthy = response.status_code in [
                200,
                404,
            ]  # 404 is ok if no batches exist

            if is_healthy:
                logger.info("✅ [BATCH-MONITOR] Bland AI API is healthy")
            else:
                logger.warning(
                    f"⚠️ [BATCH-MONITOR] Bland AI API returned status: {response.status_code}"
                )
