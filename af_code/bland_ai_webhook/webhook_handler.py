...
class WebhookHandler:
    ...
    async def handle_webhook(self, req: func.HttpRequest) -> func.HttpResponse:
        ...
        try:
            ...
            if not validation_result.is_valid:
                self.error_handler.log_validation_error(request_id, webhook_data, validation_result.errors)
                return self._error_response(request_id, "Webhook validation failed", 400,
            ...
            enrollment_update = self.business_rules.determine_enrollment_update(webhook_data, mapped_data)

            # Phase 4: Atomic Database Updates
            update_result = self.db_orchestrator.execute_atomic_updates(
                webhook_data=webhook_data, mapped_data=mapped_data, enrollment_update=enrollment_update,
                request_id=request_id
            )

            if not update_result.success:
                self.error_handler.log_database_error(request_id, "Atomic database updates",
                                                      update_result.error_message, update_result.tables_updated)
            ...
            # during the submission process and re-raise them to the main error handler.
                ...
                logger.error(f"🚨 A critical error occurred during the queue submission process: {str(e)}")
                raise
            ...
            logger.info(f"🎉 [WEBHOOK-SUCCESS] {request_id} - Webhook processing completed successfully")
            return self._success_response(
                request_id, "Webhook processed successfully", call_id,
                disposition=mapped_data.disposition, tables_updated=update_result.tables_updated
            )

        except Exception as e:
            error_message = str(e)
            stack_trace = traceback.format_exc()
            self.error_handler.log_critical_error(request_id, webhook_data, error_message, stack_trace)
    ...
    def _error_response(self, request_id: str, message: str, status_code: int,
                        errors: Optional[list] = None) -> func.HttpResponse:
    ...
    def _success_response(self, request_id: str, message: str, call_id: str, disposition: Optional[str] = None,
                          tables_updated: Optional[list] = None) -> func.HttpResponse:
        """Create a standardized success HTTP response."""
        response_body = {"status": "success", "message": message, "call_id": call_id, "request_id": request_id}
        if disposition:
