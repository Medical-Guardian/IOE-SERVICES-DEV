...
class ErrorHandler:
    ...
    def log_validation_error(self, request_id: str, webhook_data: Optional[Dict[str, Any]], errors: list) -> None:
    ...
    def log_database_error(self, request_id: str, operation: str, error_message: str,
                           tables_updated: List[str]) -> None:
        """Logs errors that occur during database operations."""
        logger.error(f"💥 [DATABASE-ERROR] Request {request_id} during '{operation}': {error_message}")
        self._log_to_database(
            request_id=request_id,
    ...
    def log_critical_error(self, request_id: str, webhook_data: Optional[Dict[str, Any]], error_message: str,
                           stack_trace: str) -> None:
        """Logs unexpected critical errors and triggers an email alert."""
        logger.critical(f"🚨 [CRITICAL-ERROR] Request {request_id}: {error_message}\nStack Trace: {stack_trace}")
        self._log_to_database(
            request_id=request_id,
    ...
    def _log_to_database(self, request_id: str, error_message: str, error_category: ErrorCategory,
                         error_severity: ErrorSeverity, context: Optional[Dict[str, Any]]) -> None:
        """
        ...
        try:
            ...
            logger.info(f"✅ [ERROR-HANDLER] Successfully logged error for request_id: {request_id} to database.")

        except Exception as e:
            logger.error(f"💥 CRITICAL: FAILED TO LOG ERROR TO DATABASE for request_id {request_id}. Error: {str(e)}")

    def _send_email_notification(self, request_id: str, error_message: str, stack_trace: str) -> None:
        ...
        try:
            msg_body = f"A critical error occurred in the Bland AI Webhook processing.\n\n" \
            ...
            logger.info(f"📧 [ERROR-HANDLER] Email notification sent successfully for request_id: {request_id}")
        except Exception as e:
            logger.error(f"💥 [ERROR-HANDLER] Failed to send email notification for request_id {request_id}: {str(e)}")
