...
class DatabaseOrchestrator:
    ...
    def execute_atomic_updates(
        ...
        # 2) Raw response insert (always) - separate audit table for large JSON payload
        raw_query, raw_params = self._build_insert_bland_raw_response(webhook_data)
        queries_to_run.append((raw_query, raw_params))
        tables_updated.append("bland_raw_response")

        # 3) Outreach attempts update (optional)
    ...
    def verify_database_constraint(self) -> None:
        ...
        try:
            ...
            if results:
                ...
                logger.info("🔍 [DB-ORCH] Database constraint verification:")
                logger.info(
                    f"🔍 [DB-ORCH]   - Constraint name: {constraint_info.get('CONSTRAINT_NAME')}"
                )
                logger.info(
                    f"🔍 [DB-ORCH]   - Table: {constraint_info.get('TABLE_SCHEMA')}.{constraint_info.get('TABLE_NAME')}"
                )
                ...
                logger.info(
                    f"🔍 [DB-ORCH]   - Raw check clause bytes: {check_clause.encode('utf-8').hex()}"
                )
                ...
                if allowed_values:
                    ...
                    for value in allowed_values:
                        logger.info(
                            f"🔍 [DB-ORCH]     * '{value}' (bytes: {value.encode('utf-8').hex()}, length: {len(value)})"
                        )
                else:
                    logger.warning("⚠️ [DB-ORCH] Could not extract allowed values from check clause")

            else:
                logger.error(
                    "❌ [DB-ORCH] Could not find constraint CK_mcee_current_status in database"
                )
    ...
    def _build_insert_bland_call_logs(self, webhook_data: Dict[str, Any]) -> Tuple[str, Tuple]:
        ...
        params = (
            ...
            None,  # raw_bland_response - now stored in bland_raw_response table (NULL)
        )
        return q, params
    ...
    def _build_insert_bland_raw_response(self, webhook_data: Dict[str, Any]) -> Tuple[str, Tuple]:
        ...
        params = (
            webhook_data.get("call_id"),
            self._safe_json(webhook_data),  # Full webhook payload as JSON string
        )
        return query, params
    ...
    def _build_update_enrollment(
        ...
        logger.info("🔍 [DB-ORCH] Processing enrollment update:")
        ...
        if not member_id or not campaign_id or not new_status:
            logger.warning("⚠️ [DB-ORCH] Enrollment update skipped - Missing required fields:")
        ...
        if is_wellness_campaign:
            if new_status.upper() == "OPTED_OUT":
                logger.info(
                    "🩺 [DB-ORCH] ✅ Wellness campaign opt-out detected - proceeding with status update to OPTED_OUT"
                )
                # Continue with normal processing to update status to OPTED_OUT
            else:
                logger.info("🩺 [DB-ORCH] ℹ️ Wellness campaign call completed successfully")
                logger.info(
                    "🩺 [DB-ORCH] ℹ️ No enrollment status change needed - member remains ENROLLED"
                )
                logger.info(
        ...
        logger.info(
            f"🔍 [DB-ORCH] Attempting to set current_status = '{new_status}' for member_id = {member_id}, campaign_id = {campaign_id}"
        )
        logger.info(
            "🔍 [DB-ORCH] Valid constraint values: OPTED_OUT, PENDING, ENROLLED, UNENROLLED"
        )
        ...
        for valid_status in valid_statuses:
            if new_status == valid_status:
                is_valid = True
                logger.info(
                    f"✅ [DB-ORCH] Status '{new_status}' matches valid status '{valid_status}' (exact match)"
                )
                break
            elif new_status.upper() == valid_status.upper():
                logger.warning(
                    f"⚠️ [DB-ORCH] Status '{new_status}' matches '{valid_status}' case-insensitively but not exactly"
                )
            else:
                ...
                if len(status_bytes) != len(valid_bytes):
                    logger.debug(
                        f"🔬 [DB-ORCH] Length mismatch: '{new_status}' ({len(status_bytes)} bytes) vs '{valid_status}' ({len(valid_bytes)} bytes)"
                    )
                else:
                    for i, (b1, b2) in enumerate(zip(status_bytes, valid_bytes)):
                        if b1 != b2:
                            logger.debug(
                                f"🔬 [DB-ORCH] Byte difference at position {i}: {b1} vs {b2} ('{chr(b1)}' vs '{chr(b2)}')"
                            )
                            break

        if not is_valid:
            logger.error(
                f"❌ [DB-ORCH] INVALID STATUS: '{new_status}' is not in allowed values: {valid_statuses}"
            )
            logger.error(
                "❌ [DB-ORCH] This will cause CHECK constraint violation CK_mcee_current_status"
            )

            # Log each valid status with byte representation for comparison
            logger.error("❌ [DB-ORCH] Valid statuses with byte representations:")
            for vs in valid_statuses:
                logger.error(
                    f"❌ [DB-ORCH]   - '{vs}': {vs.encode('utf-8').hex()} (length: {len(vs)})"
                )
        ...
        should_transition_to_wellness = is_intro_campaign and new_status == "ENROLLED"

        logger.info("🔄 [DB-ORCH] Auto-transition evaluation:")
        logger.info(f"🔄 [DB-ORCH]   - Is intro campaign: {'✅' if is_intro_campaign else '❌'}")
        logger.info(
            f"🔄 [DB-ORCH]   - Status is ENROLLED: {'✅' if new_status == 'ENROLLED' else '❌'}"
        )
        logger.info(
            f"🔄 [DB-ORCH]   - Should auto-transition: {'✅ YES' if should_transition_to_wellness else '❌ NO'}"
        )

        if should_transition_to_wellness:
            logger.info("🚀 [DB-ORCH] STARTING AUTO-TRANSITION PROCESS")
            ...
            if intro_data:
                ...
                logger.info("📊 [DB-ORCH] Intro campaign current data:")
                logger.info(f"📊 [DB-ORCH]   - Current status: {current_intro_status}")
                logger.info(f"📊 [DB-ORCH]   - Preferred window: {preferred_window}")
            else:
                logger.error(
                    f"❌ [DB-ORCH] No intro campaign record found for member {member_id}, campaign {campaign_id}"
                )
                return None
            ...
            try:
                ...
                logger.info("🔧 [DB-ORCH] Step 1/2: Updating intro campaign status to UNENROLLED")
                intro_rows = self.db_service.execute_query(
                    intro_update_q, (member_id, campaign_id), fetch_results=False
                )
                logger.info(
                    f"✅ [DB-ORCH] Step 1/2 completed: {intro_rows} intro campaign rows updated"
                )

                # Log intro campaign status change: ENROLLED -> UNENROLLED
                if intro_rows > 0:
                    logger.info(
                        "📋 [DB-ORCH] Logging intro campaign status change to audit history"
                    )
                    ...
                    logger.info("✅ [DB-ORCH] Intro campaign audit log completed")

                # STEP 2: Create/Update wellness campaign to ENROLLED
                logger.info("🔧 [DB-ORCH] Step 2/2: Creating/updating wellness campaign enrollment")
                logger.info(
                    f"🔧 [DB-ORCH] Using preferred_window '{preferred_window}' from intro campaign {campaign_id}"
                )
                wellness_rows = self.db_service.execute_query(
                    wellness_upsert_q,
                    (member_id, WELLNESS_CAMPAIGN_ID, "ENROLLED", preferred_window),
                    fetch_results=False,
                )
                logger.info(
                    f"✅ [DB-ORCH] Step 2/2 completed: {wellness_rows} wellness campaign rows affected"
                )
                ...
                if wellness_rows > 0:
                    ...
                    previous_wellness_status = (
                        existing_wellness_records[0].get("current_status")
                        if existing_wellness_records
                        else None
                    )

                    logger.info(
                        f"📊 [DB-ORCH] Wellness campaign previous status: {previous_wellness_status if previous_wellness_status else 'NEW_RECORD'}"
                    )
                    ...
                    logger.info("✅ [DB-ORCH] Wellness campaign audit log completed")

                logger.info("🎉 [DB-ORCH] AUTO-TRANSITION COMPLETED SUCCESSFULLY!")
                logger.info("🎉 [DB-ORCH] Summary:")
                ...
                logger.info(
                    f"🎉 [DB-ORCH]   - Final transition: {campaign_id} (UNENROLLED) → {WELLNESS_CAMPAIGN_ID} (ENROLLED)"
                )
            ...
            logger.info("📝 [DB-ORCH] Processing standard enrollment status update")
            ...
            try:
                ...
                current_status = (
                    current_records[0].get("current_status") if current_records else None
                )

                logger.info(
                    f"📊 [DB-ORCH] Current enrollment status: {current_status if current_status else 'NOT_FOUND'}"
                )
                ...
                logger.info(
                    f"✅ [DB-ORCH] Enrollment update completed - {rows_affected} rows affected"
                )

                # Log status change if update actually happened
                if rows_affected > 0 and current_status != new_status:
                    logger.info("📋 [DB-ORCH] Logging status change to audit history")
                    self.log_status_change(
                        member_id=member_id,
                        campaign_id=campaign_id,
                    ...
                    logger.info("✅ [DB-ORCH] Status change audit logged successfully")
                else:
                    logger.info(
                        "ℹ️ [DB-ORCH] No status change audit needed (no rows updated or status unchanged)"
                    )

                # Final validation before sending to database
                logger.info(
                    f"🔍 [DB-ORCH] Final status value being sent to database: {repr(new_status)}"
                )
    ...
    def _handle_campaign_auto_transition(
        ...
        should_transition_to_wellness = (
            campaign_id.upper() == INTRO_CAMPAIGN_ID.upper() and new_status == "ENROLLED"
        )

        if not should_transition_to_wellness:
            logger.info(
                "ℹ️ [DB-ORCH] No auto-transition needed - not intro campaign or status not ENROLLED"
            )
            return False
        ...
        try:
            ...
            # Log intro campaign status change: ENROLLED -> UNENROLLED
            if intro_rows > 0:
                logger.info("📋 [DB-ORCH] Logging intro campaign status change to audit history")
                self.log_status_change(
                    member_id=member_id,
                    campaign_id=campaign_id,
                    previous_status="ENROLLED",
        ...
        try:
            ...
            if previous_status:
                ...
                last_change_records = self.db_service.execute_query(
                    last_change_query, (member_id, campaign_id)
                )
            ...
            logger.info(
                f"📋 [DB-ORCH] Status change logged: {member_id} {previous_status}→{new_status} ({change_source})"
            )
