...
class StatusMapper:
    ...
    def __init__(self) -> None:
        ...
        logger.info(
            f"📋 [STATUS-MAPPER] Loaded {len(self.status_disposition_mapping)} specific mapping rules"
        )
    ...
    def map_webhook_to_internal_format(self, webhook_data: Dict[str, Any]) -> MappedCallData:
        ...
        logger.info(
            f"📊 [STATUS-MAPPER] Core status - Status: '{status}', Disposition: '{disposition_tag}'"
        )
        ...
        logger.info(
            f"📋 [STATUS-MAPPER] Mapped results - Disposition: '{disposition}', Next Action: '{next_action}'"
        )
        ...
        logger.info(
            f"📝 [STATUS-MAPPER] Response summary created: {len(response_summary)} characters"
        )
    ...
    def _build_response_summary(
        self, webhook_data: Dict[str, Any], disposition: str, contact_made: bool
    ) -> str:
        """
        ...
        summary_parts.append(
            "Contact was successfully made with the member"
            if contact_made
            else "No contact was made with the member"
        )
