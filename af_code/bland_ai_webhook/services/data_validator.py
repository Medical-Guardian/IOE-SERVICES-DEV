...
class DataValidator:
    ...
    def __init__(self, config_manager: ConfigManager = None):
        ...
        logger.info(f"🔍 [DATA-VALIDATOR] Data validator initialized with max status length: {self.max_status_length}")
    ...
    def validate_webhook_payload(self, webhook_data: Dict[str, Any]) -> ValidationResult:
        ...
        if not to_phone_number or not isinstance(to_phone_number, str) or not self.phone_regex.match(to_phone_number):
        ...
        if not isinstance(metadata, dict):
            ...
            if missing_metadata_fields:
                errors.append(f"Missing required metadata fields: {', '.join(missing_metadata_fields)}")

        # Validate status
        status = webhook_data.get('status', '').lower()
        valid_statuses = {'completed', 'failed', 'in-progress', 'cancelled'}
        if status not in valid_statuses:
            warnings.append(f"Unknown status '{status}', expected one of {', '.join(valid_statuses)}")
        ...
        logger.info(f"{'✅' if is_valid else '❌'} [DATA-VALIDATOR] Validation {'succeeded' if is_valid else 'failed'}")
        if errors:
