...
def handle_blob_movement_with_error_handling(source_filename: str, source_folder: str, target_folder: str, container_name: str, logger):
    """
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Successfully moved {source_filename} from {source_folder} to {target_folder}")

        except Exception as blob_check_error:
            if "BlobNotFound" in str(blob_check_error) or "does not exist" in str(blob_check_error):
                logger.warning(f"⚠️ Source blob {source_blob_path} not found, checking other locations...")
                ...
                for check_folder in ["landing", "staging", "processed"]:
                    ...
                    try:
                        ...
                        logger.info(f"✅ Found and moved {source_filename} from {check_folder} to {target_folder}")
                        return

                    except:
                        continue  # Keep looking

                logger.error(f"❌ Could not find {source_filename} in any folder (landing, staging, processed)")
            else:
...
def get_dtc_schema() -> DataFrameSchema:
    ...
    return DataFrameSchema({
        ...
        "language_pref": Column(str, nullable=True, checks=Check.isin(["EN", "ES", "Other", None])),
        "salesforce_account_number": Column(str, nullable=False, checks=Check.str_length(min_value=1)),
        ...
        "caregiver_first_name": Column(str, nullable=True, checks=Check.str_matches(r"^[A-Za-z ]+$", ignore_na=True)),
        "caregiver_last_name": Column(str, nullable=True, checks=Check.str_matches(r"^[A-Za-z ]+$", ignore_na=True)),
        ...
        "is_device_callable": Column(str, nullable=True, checks=Check.isin(["Y", "N", "Yes", "No", "True", "False", "1", "0", None])),
        "device_phone_number": Column(str, nullable=True),
        "checkin_time": Column(str, nullable=True, checks=Check.isin(["AM", "PM", "EV", None])),
        "enrollment_status": Column(str, nullable=False, checks=Check.isin(["enroll", "update", "unenroll"])),
...
def validate_timezone(timezone_str: str) -> Optional[str]:
    ...
    timezone_mappings = {
        ...
        'SST': 'Pacific/Samoa', 'SAMOA': 'Pacific/Samoa',
        'CHST': 'Pacific/Guam', 'CHAMORRO': 'Pacific/Guam',
...
class DatabaseManager:
    ...
    def __init__(self,
                 secret_or_conn: str,
                 key_vault_url_env: str = "KEY_VAULT_URL"):
        ...
        if ";" in secret_or_conn and "=" in secret_or_conn:
            ...
            self.logger.debug(f"Initialized with Key Vault secret '{self.secret_name}' at {self.key_vault_url}")

    def _fetch_connection_string(self) -> str:
        """Pulled only when using Key Vault mode."""
        try:
            self.logger.info(f"🔐 Fetching secret '{self.secret_name}' from Key Vault: {self.key_vault_url}")
            credential = DefaultAzureCredential()
...
def extract(file_path: str, context: DTCProcessingContext) -> Tuple[pd.DataFrame, ProcessingResult]:
    ...
    try:
        ...
        df = download_blob_as_dataframe(f"landing/{context.source_filename}", os.environ["AZURE_CONTAINER_NAME"])
...
def load_to_staging(df: pd.DataFrame, context: DTCProcessingContext) -> ProcessingResult:
    ...
    try:
        ...
        cursor = db_manager.execute_with_retry(context.connection, cleanup_sql, (str(context.file_batch_id),))
        ...
        if error_rate > context.config.error_threshold_pct:
            raise ValidationError(f"Error rate {error_rate:.2f}% exceeds threshold {context.config.error_threshold_pct}%")
        ...
        staging_columns = [
            'file_batch_id', 'source_filename', 'row_number_in_file', 'load_timestamp',
            'file_load_date', 'processing_status', 'error_message', 'campaign_id',
            # 🔥 ADD METADATA COLUMNS:
            'file_size_bytes', 'total_rows_in_file', 'uploaded_by_user',
            'cleansing_started_ts', 'cleansing_completed_ts', 'enrollment_started_ts',
            # Raw CSV columns
            ...
            'customer_timezone', 'member_dob', 'member_email', 'member_address_street',
            ...
            'device_udi', 'device_name', 'is_device_callable', 'device_phone_number',
            'checkin_time', 'enrollment_status', 'unenrollment_reason',
            # Clean columns
            'first_name_clean', 'last_name_clean', 'primary_phone_clean',
            'caregiver_first_clean', 'caregiver_last_clean', 'caregiver_phone_clean', 'caregiver_email_clean',
            'device_phone_clean', 'dob_clean', 'timezone_clean', 'is_device_callable_clean'
        ]
...
def validate_and_cleanse_data_before_insert(df: pd.DataFrame, context: DTCProcessingContext) -> Tuple[pd.DataFrame, List[Dict]]:
    """
    ...
    for idx, row in df_clean.iterrows():
        ...
        if not partner_name:
            row_errors.append("Missing required partner_name")
        ...
        if not sf_account:
            ...
            try:
                ...
                if not sf_number.isdigit():
                    ...
                    row_errors.append(f"salesforce_account_number invalid length: '{sf_account}' (4-15 digits expected)")
            except:
        ...
        if not enrollment_status:
            ...
            row_errors.append(f"Invalid enrollment_status: '{enrollment_status}' (must be: {valid_statuses})")
        df_clean.loc[idx, 'enrollment_status'] = enrollment_status.lower() if enrollment_status else None
        ...
        if language_pref and language_pref.upper() not in valid_languages:
            row_errors.append(f"Invalid language_pref: '{language_pref}' (must be: {valid_languages})")
        df_clean.loc[idx, 'language_pref'] = language_pref.upper() if language_pref else 'EN'  # Default to EN
        ...
        if dob:
            try:
                ...
                if not parsed_date:
                    row_errors.append(f"Invalid date format for member_dob: '{dob}' (expected YYYY-MM-DD)")
        ...
        if checkin_time:
            if checkin_time.upper() not in valid_checkin_times:
                row_errors.append(f"Invalid checkin_time: '{checkin_time}' (must be: {valid_checkin_times})")
        ...
        if enrollment_status and enrollment_status.lower() == 'unenroll':
            if not unenroll_reason:
                row_errors.append("unenrollment_reason required when enrollment_status = 'unenroll'")
        ...
        for addr_field in ['member_address_street', 'member_address_city', 'member_address_state',
                          'member_address_zip', 'member_address_country']:
        ...
        if not member_phone and not email and not device_phone:
            row_errors.append("At least one contact method required (member_phone, email, or device_phone)")
...
def pre_insert_final_validation(df_clean: pd.DataFrame) -> Tuple[bool, List[str]]:
    ...
    for field in critical_fields:
        empty_count = len(df_clean[df_clean[field].isin(['', None])])
        if empty_count > 0:
            final_errors.append(f"Found {empty_count} empty values in critical field: {field}")
...
def validate_data(context: DTCProcessingContext) -> ProcessingResult:
    ...
    try:
        ...
        cursor = db_manager.execute_with_retry(context.connection, update_sql, (str(context.file_batch_id),))
        ...
        cursor = db_manager.execute_with_retry(context.connection, cleansing_sql, (str(context.file_batch_id),))
        ...
        cursor = db_manager.execute_with_retry(context.connection, mark_validated_sql, (str(context.file_batch_id),))
        ...
        cursor = db_manager.execute_with_retry(context.connection, result_sql, (str(context.file_batch_id),))
...
def transform_and_load_core(context: DTCProcessingContext) -> ProcessingResult:
    ...
    try:
        ...
        """
        cursor = db_manager.execute_with_retry(context.connection, populate_org_sql, (str(context.file_batch_id),))
        ...
        if not rows:
            raise PermanentError("❌ No active campaigns found for dtc_intro_onboarding or dtc_wellness_check")
        ...
        """
        cursor = db_manager.execute_with_retry(context.connection, update_status_sql, (str(context.file_batch_id),))
        ...
        """
        cursor = db_manager.execute_with_retry(context.connection, check_staging_sql, (str(context.file_batch_id),))
        staging_check = cursor.fetchone()

        logger.info(f"Staging data check:")
        ...
        cursor = db_manager.execute_with_retry(context.connection, member_upsert_sql, (str(context.file_batch_id),))
        ...
        WHERE stg.file_batch_id = %s
        """
        cursor = db_manager.execute_with_retry(context.connection, verify_members_sql, (str(context.file_batch_id),))
        ...
        WHERE file_batch_id = %s AND processing_status = 'TRANSFORMING'
        """
        cursor = db_manager.execute_with_retry(context.connection, update_enrollment_start_sql,
        ...
        """
        cursor = db_manager.execute_with_retry(context.connection, unenroll_sql, (
        ...
        """
        cursor = db_manager.execute_with_retry(context.connection, device_sql, (str(context.file_batch_id),))
        ...
        """
        cursor = db_manager.execute_with_retry(context.connection, update_tracking_sql, (str(context.file_batch_id),))
        ...
        WHERE file_batch_id = %s AND processing_status = 'TRANSFORMING'
        """
        cursor = db_manager.execute_with_retry(context.connection, complete_sql, (str(context.file_batch_id),))
        ...
        logger.info(f"   Final counts - Members today: {final_counts[0]}, Enrollments today: {final_counts[1]}")
...
def log_audit(context: DTCProcessingContext, processing_results: List[ProcessingResult]) -> ProcessingResult:
    """
    ...
    try:
        ...
        cursor = db_manager.execute_with_retry(context.connection, audit_sql,
                                             (total_processed, total_succeeded, total_failed,
                                              total_processed, total_failed, total_processed, str(context.file_batch_id)))
        ...
        cursor = db_manager.execute_with_retry(context.connection, audit_detail_sql,
                                             (str(context.file_batch_id), total_processed, total_succeeded,
                                              total_failed, audit_message, datetime.now(timezone.utc)))
...
def handle_errors(context: DTCProcessingContext, error: Exception, processing_results: List[ProcessingResult]) -> None:
    """
    ...
    try:
        ...
        check_sql = "SELECT COUNT(*) FROM engage360_stg.file_processing_log WHERE file_batch_id = %s"
        ...
        if cursor.fetchone()[0] > 0:
            ...
            try:
                ...
                cursor.execute(error_detail_sql,
                              (str(context.file_batch_id),
                              error_message,
                              str(error)))
                context.connection.commit()
...
def process_dtc_file_complete(
    ...
    logger.info(f"Expected pattern:   MedicalGuardian_DTCWellness_*_Delta.csv")

    # -------------------------------------------------------------------------
    # Validate filename
    # -------------------------------------------------------------------------
    if not source_filename.startswith("MedicalGuardian_DTCWellness_") \
    ...
    try:
        ...
        if result.success:
            try:
                handle_blob_movement_with_error_handling(
                    source_filename, "landing", "staging",
                    os.environ["AZURE_CONTAINER_NAME"], logger
                )
            except Exception as move_error:
        ...
        if result.success:
            try:
                handle_blob_movement_with_error_handling(
                    source_filename, "staging", "processed",
                    os.environ["AZURE_CONTAINER_NAME"], logger
                )
            except Exception as move_error:
        ...
        summary = {
            ...
            "workflow_steps": [
                ...
                for i, (name, r) in enumerate(zip(
                    ["EXTRACT", "LOAD_TO_STAGING", "VALIDATE_DATA",
                     "TRANSFORM_AND_LOAD_CORE", "LOG_AUDIT"],
                    processing_results
                ))
            ]
        }
...
if __name__ == "__main__":
    ...
    if success:
        ...
        for step in details['workflow_steps']:
            ...
            print(f"{status} Step {step['step']} ({step['name']}): {step['message']} - {step['duration_seconds']:.2f}s")
    else:
