...
| **Azure SQL Database** | Primary data store (ioe schema) | High ($200-500/mo) | Yes | All functions |
...
**Reference**: See `IOE_TABLE_USAGE_REFERENCE.md` for complete schema
...
3. **Index Optimization**:
   - Current: Missing indexes on frequently queried columns (see `IOE_TABLE_USAGE_REFERENCE.md`)
   - **Add indexes**:
     ```sql
     -- High-impact indexes
...
conn_string = f"Server=tcp:your-server.database.windows.net,1433;Database=ioe;Authentication=ActiveDirectoryIntegrated;"
```
...
|-------------|--------|---------|-------------------|
| `SqlConnectionStringIOE` | Azure SQL connection string | `Server=tcp:your-server.database.windows.net,1433;Database=ioe;User ID=...;Password=...;Encrypt=yes;` | 90 days |
...
az monitor diagnostic-settings create \
  ...
  --resource /subscriptions/<sub-id>/resourceGroups/<rg>/providers/Microsoft.Sql/servers/<sql-server>/databases/ioe \
