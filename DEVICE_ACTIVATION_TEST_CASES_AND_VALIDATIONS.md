...
- Call Scheduling Logic: 18 test cases
...
**Implementation:** Date parsing and age validation in data cleansing logic
...
- **TC-DA-SCH-003-04:** Call 5 timestamp + 91 days → Verify member NOT eligible (outside 90-day window)
...
### 20. 90-Day Campaign Window (Call 5+)

**Rule:** The system must enforce 90-day campaign window starting from call_5_timestamp. Calls after Day 90 are not eligible.

**Description:** Campaign window is 90 calendar days FROM when Call 5 is created (call_5_timestamp), not from activation_start_date.

**Test Cases:**
- **TC-DA-SCH-005-01:** Day 1 after Call 5 created → Verify member eligible
- **TC-DA-SCH-005-02:** Day 89 after Call 5 created → Verify member eligible
- **TC-DA-SCH-005-03:** Day 90 after Call 5 created → Verify member eligible (last day)
- **TC-DA-SCH-005-04:** Day 91 after Call 5 created → Verify member NOT eligible (expired)
- **TC-DA-SCH-005-05:** call_5_timestamp = NULL (Calls 1-4) → Verify no 90-day limit applied

**Expected Behavior:** 90-day window enforced only after Call 5 timestamp is set

**Database Impact:** `member_campaign_enrollments_enhanced.call_5_timestamp` + 90 days = campaign_end_date

**Implementation:** Eligibility query: `WHERE campaign_end_date IS NULL OR SYSDATETIMEOFFSET() <= campaign_end_date`
...
### 23. Batch Size Limit (100 Members)

**Rule:** The system must limit batch size to 100 members per Bland AI batch submission.

**Description:** If >100 eligible members, create multiple batches of max 100 each.

**Test Cases:**
- **TC-DA-SCH-008-01:** 50 eligible members → Verify 1 batch created with 50 members
- **TC-DA-SCH-008-02:** 100 eligible members → Verify 1 batch created with 100 members
- **TC-DA-SCH-008-03:** 150 eligible members → Verify 2 batches created (100 + 50)
- **TC-DA-SCH-008-04:** 250 eligible members → Verify 3 batches created (100 + 100 + 50)

**Expected Behavior:** Batch size never exceeds 100 members

**Database Impact:** Multiple `outreach_batches` records created if needed

**Implementation:** `batch_orchestrator.py` chunks members into batches of 100
...
| Call Scheduling Logic | 18 | Business days, frequency, timezones, callbacks |
...
### Test Execution
1. **CSV File Tests:** Upload test CSV files to fs-device-activation/landing/ or fs-ops/landing/
