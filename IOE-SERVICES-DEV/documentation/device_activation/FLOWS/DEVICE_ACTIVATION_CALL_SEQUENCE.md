...
- **Calls 1-4 (Initial Phase):** Frequent attempts using BUSINESS days (2 days for Calls 2-3, 5 days for Call 4) with a maximum of 4 attempts.
- **Calls 5+ (Extended Phase):** After 7 days (>7 CALENDAR days = 8+ days frequency, calls only on business days) within a 90-day window starting from activation_start_date (updated 2026-01-17).
...
4. **90-Day Campaign Window (UPDATED 2026-01-17):**
   - **All calls (1-5+)** are subject to the 90-day campaign window
   - 90-day window starts from activation_start_date (when Call 1 is eligible)
   - campaign_end_date = activation_start_date + 90 days (set at enrollment time)
   - Members are automatically unenrolled when campaign_end_date expires (hourly campaign closure)
   - call_5_timestamp field is deprecated (no longer used for campaign end calculation)
...
### Purpose
Shows the weekly call frequency and 90-day window for Call 5 and beyond. **UPDATED 2026-01-17:** The 90-day window now starts from activation_start_date (not call_5_timestamp), and campaign_end_date is set at enrollment time.

### Mermaid Diagram

```mermaid
gantt
    title Device Activation Call 5+ Timeline (90-Day Window - UPDATED 2026-01-17)
    dateFormat YYYY-MM-DD
    axisFormat %m/%d

    section Enrollment & Campaign Window
    activation_start_date (Call 1 eligible) :milestone, start, 2025-01-03, 0d
    campaign_end_date = start + 90 days :milestone, end, 2025-04-03, 0d

    section Call 5+ Weekly Frequency (7-day calendar spacing)
    Call 5 Eligible          :c5, 2025-01-16, 1d
    Call 6 Eligible (7d later) :c6, 2025-01-23, 1d
    Call 7 Eligible (7d later) :c7, 2025-01-30, 1d
    Call 8 Eligible (7d later) :c8, 2025-02-06, 1d
    Call N (continues weekly) :cn, 2025-02-13, 49d

    section Campaign Closure
    90-Day Window Expires    :milestone, expire, 2025-04-03, 0d
    Hourly Campaign Closure Unenrolls Member :crit, nomore, 2025-04-03, 1d
```

### ASCII Diagram (UPDATED 2026-01-17)

```
Day 1 (Jan 3):      ✅ activation_start_date (Call 1 eligible)
                    📅 campaign_end_date = Jan 3 + 90 days = Apr 3 (set at enrollment)
                        ↓
                        ... (Calls 1-4 as per business day schedule) ...
                        ↓
Day 13 (Jan 16):    📞 Call 5 Eligible & Made
                        ↓
                        (7 calendar days)
                        ↓
Day 20 (Jan 23):    📞 Call 6 Eligible & Made
                        ↓
                        (7 calendar days)
                        ↓
Day 27 (Jan 30):    📞 Call 7 Eligible & Made
                        ↓
                        (7 calendar days)
                        ↓
Day 34 (Feb 6):     📞 Call 8 Eligible & Made
                        ↓
                        ...
                        ↓
Day 90 (Apr 3):     🛑 Campaign Ends (campaign_end_date reached)
                    ⏰ Hourly campaign closure scheduler unenrolls member
                    ❌ No more calls eligible
                    📊 Enrollment status updated to 'UNENROLLED' (BC-DA-007)
...
• Max Attempts: Unlimited (within 90-day window from activation_start_date)
• Window Start: activation_start_date (when Call 1 is eligible) - UPDATED 2026-01-17
• Window Duration: Exactly 90 calendar days
• Window End: activation_start_date + 90 days = campaign_end_date (set at enrollment)
• Eligibility: SYSDATETIMEOFFSET() <= campaign_end_date
• Campaign Closure: Hourly timer (BC-DA-007) auto-unenrolls expired members
• Max Possible Calls: Depends on delivery date (typically 10-13 total calls)
```

### Key Points (UPDATED 2026-01-17)

1. **campaign_end_date Calculation (NEW LOGIC):**
   - **CRITICAL:** Window starts from `activation_start_date`, NOT from `call_5_timestamp`
   - Formula: `campaign_end_date = activation_start_date + 90 days`
   - Set at enrollment creation (Phase 4 of file processing - Transform)
   - Stored in `member_campaign_enrollments_enhanced.campaign_end_date`
   - Example: activation_start_date = Jan 3 → campaign_end_date = Apr 3
   - Window is 90 **calendar days** (includes weekends/holidays)
   - Code: `af_code/af_device_activation_logic.py` (enrollment INSERT)

2. **call_5_timestamp Field (DEPRECATED):**
   - Field remains in database but is NO LONGER USED for campaign end calculation
   - New enrollments do not set call_5_timestamp
   - Legacy logic preserved for backward compatibility only
   - Campaign closure (BC-DA-007) does NOT reference call_5_timestamp
...
4. **Unlimited Attempts Within Window:**
   - No maximum attempt count for Call 5+ (unlike Calls 1-4 with max 4 attempts)
   - Total calls depend on how quickly Calls 1-4 are completed
   - Typical range: 10-13 total calls (4 initial + 6-9 weekly)
   - Actual count varies based on delivery date, business hours availability, and call outcomes
...
5. **Campaign End Date Enforcement:**
   ...
   - Campaign closure scheduler (BC-DA-007) runs hourly, unenrolls expired members
   - Member status updated to 'UNENROLLED' when campaign_end_date expires
   - Code: `af_code/device_activation_scheduler/services/eligibility_service.py:123-127`
   - Code: `functions/device_activation_campaign_closure.py` (hourly unenrollment)

6. **Why Window Starts from activation_start_date (UPDATED 2026-01-17):**
   - Ensures consistent 90-day window for all members regardless of call pacing
   - Prevents indefinite calling while allowing reasonable follow-up duration
   - Simplifies campaign closure logic (no dependency on Call 5 timing)
   - Business rationale: 90 days from device activation eligibility is sufficient for outreach
   - Hourly campaign closure automatically cleans up expired enrollments

### Example Timeline Calculation (UPDATED 2026-01-17)

```
Scenario: Member receives device on Jan 1 (Wednesday)

Jan 1:  Device delivery (Wednesday)
Jan 3:  activation_start_date = delivery + 2 biz days (Friday)
        campaign_end_date = Jan 3 + 90 days = Apr 3 (set at enrollment)
        📞 Call 1 Eligible & Made
...
Jan 23: 📞 Call 5 (Thursday - 7 CALENDAR days later)
Jan 30: 📞 Call 6 (7 calendar days later)
Feb 6:  📞 Call 7 (7 calendar days later)
Feb 13: 📞 Call 8 (7 calendar days later)
Feb 20: 📞 Call 9 (7 calendar days later)
Feb 27: 📞 Call 10 (7 calendar days later)
Mar 6:  📞 Call 11 (7 calendar days later)
Mar 13: 📞 Call 12 (7 calendar days later)
Mar 20: 📞 Call 13 (7 calendar days later)
Mar 27: 📞 Call 14 (7 calendar days later)
Apr 3:  🛑 Campaign ends (campaign_end_date reached = activation_start_date + 90 days)
        ⏰ Hourly campaign closure scheduler unenrolls member
Apr 4:  ❌ No longer eligible (SYSDATETIMEOFFSET() > campaign_end_date)

Total Calls: 14 (4 initial + 10 weekly within 90-day window)
Total Duration: 90 days (Jan 3 to Apr 3 - activation_start_date to campaign_end_date)
```

### Related Code Files

- **campaign_end_date Calculation:** `af_code/af_device_activation_logic.py` (enrollment INSERT during file processing)
- **Campaign Closure Scheduler:** `functions/device_activation_campaign_closure.py` (BC-DA-007, hourly timer)
- **Call 5 Timestamp Update (DEPRECATED):** `af_code/device_activation_scheduler/services/batch_orchestrator.py:_update_call_5_enrollments()`
