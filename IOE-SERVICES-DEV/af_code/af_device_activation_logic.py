...
def validate_and_cleanse_data_before_insert(
    ...
    # ===================================================================
    # TODO: Device Ownership Validation (DISABLED - KeyError Fix)
    # ===================================================================
    # ISSUE: Device ownership validation was causing KeyError: 'member_id'
    # ROOT CAUSE: member_id doesn't exist in DataFrame at Phase 2 (Load to Staging)
    #             - CSV has salesforce_account_number, not member_id
    #             - member_id is a database UUID populated later in Phase 4
    # PROPER FIX: Move validation to Phase 3 (validate_data) or Phase 4 (transform_data)
    #             using SQL-based approach (similar to DTC pattern in af_dtc_logic.py:2305-2390)
    # SQL APPROACH:
    #   1. Join stg_device_activation_delta with members table (get member_id from salesforce_account_number)
    #   2. Join with member_devices table (check if device_udi already assigned to different member)
    #   3. Flag conflicts and update staging table error_message
    # REFERENCE: See af_dtc_logic.py lines 2305-2390 for working implementation
    logger.info("ℹ️ [DEVICE-OWNERSHIP] Device ownership validation deferred to Phase 3/4 (TODO)")
