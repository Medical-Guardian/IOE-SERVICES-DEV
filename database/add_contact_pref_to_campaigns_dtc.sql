/*
Database Migration: Add contact_pref Column to ioe.campaigns_enhanced Table
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID('ioe.campaigns_enhanced')
    AND name = 'contact_pref'
)
BEGIN
    PRINT '📋 Adding contact_pref column to ioe.campaigns_enhanced table...';
...
ELSE
BEGIN
    PRINT 'ℹ️ contact_pref column already exists in ioe.campaigns_enhanced';
END
GO
