/*
Database Migration: Add to_number Column to ioe.bland_call_logs Table
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.columns
    WHERE object_id = OBJECT_ID('ioe.bland_call_logs')
    AND name = 'to_number'
)
BEGIN
    PRINT '📋 Adding to_number column to ioe.bland_call_logs table...';

    ALTER TABLE ioe.bland_call_logs
...
ELSE
BEGIN
    PRINT 'ℹ️ to_number column already exists in ioe.bland_call_logs';
END
GO
...
PRINT '======================================';
PRINT '';
PRINT 'Column added: ioe.bland_call_logs.to_number';
...
PRINT '  ALTER TABLE ioe.bland_call_logs DROP COLUMN to_number;';
PRINT '';
