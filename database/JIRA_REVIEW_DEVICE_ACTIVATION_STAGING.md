...
1. Backup existing table (if exists):
   ```sql
   SELECT * INTO ioe_stg.stg_device_activation_delta_backup
...
- ✅ Affects both production (`ioe.member_devices`) and staging tables
- ⏳ **REQUIRED ACTION**: Run migration script on Azure SQL Database
...
1. **Backup existing table** (if exists):
   ```sql
   SELECT * INTO ioe_stg.stg_device_activation_delta_backup_20251224
   FROM ioe_stg.stg_device_activation_delta;
   ```
